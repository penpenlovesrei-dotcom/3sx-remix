...
const void (*effmovejptbl[229])() = {
    ...
    // Slot 28 was a dummy and is now the COLOR EDIT preview: its sprite has to be pushed from
    // inside the effect lists, where the streaming texture cache is open. See ColEdit_Move.
    effect_24_move,    effect_25_move,    effect_26_move,    effect_27_move,    ColEdit_Move,      effect_29_move,
